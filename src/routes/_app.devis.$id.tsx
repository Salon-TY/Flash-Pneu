...
import { ArrowLeft, Download, Trash2, Pencil, Plus, FileText } from "lucide-react";
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex h-10 items-center rounded-lg border border-border bg-muted/30 px-3 text-sm font-medium">
                    {formatEUR(
                      (Number(lines[i]?.quantite) || 0) * (Number(lines[i]?.prix_unitaire_ht) || 0),
                    )}
                  </div>
                </div>
              </div>
...
function DevisDetail() {
  ...
  if (!quote)
    return <div className="py-10 text-center text-sm text-destructive">Devis introuvable.</div>;
    ...
    <PageContainer>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="break-words text-xl font-bold">{quote.numero}</h1>
          <Badge className={cn(STATUT_COLORS[quote.statut] ?? "bg-muted text-muted-foreground")}>
        ...
        <Card>
          <CardContent className="p-3 flex flex-wrap gap-2">
            ...
            {quote.statut !== "envoye" && quote.statut !== "accepte" && (
              <Button size="sm" variant="outline" onClick={() => handleStatut("envoye")}>
                Marquer envoyé
              </Button>
            )}
            <Button
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-muted-foreground mt-0.5">
                  {l.quantite} × {formatEUR(l.prix_unitaire_ht)}
                </div>
              </div>
          ...
          <div className="rounded-xl bg-muted/30 p-3 space-y-1 text-sm border-t">
            ...
            <div className="flex justify-between">
              <span className="text-muted-foreground">TVA ({quote.tva_taux}%)</span>
              <span>{formatEUR(quote.tva)}</span>
            </div>
