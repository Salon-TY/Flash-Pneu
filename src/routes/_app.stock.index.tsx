  ...
  const filteredProducts = useMemo(() => {
    ...
    return products
      .filter(
        (p) =>
          !s ||
          [p.nom, p.dimension, p.marque, p.modele].some((v) => v?.toLowerCase().includes(s)),
      )
      .filter((p) => saisonFilter === "all" || p.saison === saisonFilter)
      .sort((a, b) => a.nom.localeCompare(b.nom));
  }, [products, q, saisonFilter]);
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <div className="relative max-w-2xl flex-1">
            ...
            <Input
              ...
              placeholder="Dimension, marque, modèle…"
              className="pl-9"
            />
          </div>
      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h3 className="break-words font-semibold leading-snug">{product.nom}</h3>
            {(product.dimension || product.marque || product.modele) && (
              <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-sm">
                {product.dimension && <span className="font-medium">{product.dimension}</span>}
                ...
                {product.etat === "occasion" && (
                  <span className="rounded-full bg-warning/15 px-1.5 py-0.5 text-[9px] font-semibold uppercase text-warning-foreground">
                    Occasion
                  </span>
                )}
                {product.etat === "neuf" && product.dimension && (
              ...
              {product.dimension && (
                <>
                  ...
                  <span className="capitalize">
                    {SAISONS_PNEU.find((s) => s.value === product.saison)?.label ?? product.saison}
                  </span>
                </>
              )}
              <span
...
function ProductForm({
  ...
  const form = useForm<StockProductForm>({
    ...
    defaultValues: {
      ...
      saison: defaultValues?.saison ?? "ete",
      indice_charge: defaultValues?.indice_charge ?? "",
      indice_vitesse: defaultValues?.indice_vitesse ?? "",
      dot: defaultValues?.dot ?? "",
      etat: defaultValues?.etat ?? "neuf",
      diametre_pouce: defaultValues?.diametre_pouce,
  ...
  // Auto-calcule le diamètre en pouces depuis la dimension (ex. "205/55 R16" → 16).
  function handleDimensionChange(v: string) {
    form.setValue("dimension", v);
    const diametre = extraireDiametrePouce(v);
    if (diametre) form.setValue("diametre_pouce", diametre);
  }

  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
      <Field label="Nom du produit *" error={form.formState.errors.nom?.message}>
        <Input placeholder="Ex. Michelin Primacy 4 205/55 R16" {...form.register("nom")} />
      </Field>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Diamètre (pouces)">
          <Input type="number" min="10" max="30" {...form.register("diametre_pouce")} />
        </Field>
        <Field label="Marque">
          <Input placeholder="Michelin" {...form.register("marque")} />
        </Field>
        <Field label="Modèle">
          <Input placeholder="Primacy 4" {...form.register("modele")} />
        </Field>
        <Field label="Saison">
          <Select value={form.watch("saison")} onValueChange={(v) => form.setValue("saison", v)}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <Field label="État">
          <Select value={form.watch("etat")} onValueChange={(v) => form.setValue("etat", v)}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <Field label="Indice de charge">
          <Input placeholder="91" {...form.register("indice_charge")} />
        </Field>
        <Field label="Indice de vitesse">
          <Input placeholder="V" {...form.register("indice_vitesse")} />
        </Field>
        <Field label="DOT">
          <Input placeholder="2324" {...form.register("dot")} />
        </Field>
        <Field label="Emplacement">
          <Input placeholder="Rayon A3" {...form.register("emplacement")} />
        </Field>
      </div>
