...
export const Route = createFileRoute("/_app/factures/new")({
  head: () => ({ meta: [{ title: `Nouvelle facture — ${APP_NAME}` }] }),
  ...
  return (
    <PageContainer>
      ...
      <PageHeader
        ...
        subtitle="Renseignez le client, les prestations et l’échéance."
      />
          ...
          <CardContent className="p-4 space-y-3">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field
                ...
                error={(form.formState.errors as any).date_facture?.message}
              >
          ...
          <CardContent className="p-4 space-y-3">
            ...
            {(form.formState.errors as any).lines?.root && (
              <p className="text-xs text-destructive">
                {(form.formState.errors as any).lines.root.message}
              </p>
            )}
            {fields.map((field, i) => (
              <div key={field.id} className="rounded-lg border border-border p-3 space-y-2">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <Field
                    label="Quantité"
                    error={(form.formState.errors as any).lines?.[i]?.quantite?.message}
                  >
                    <Input type="number" step="0.01" {...form.register(`lines.${i}.quantite`)} />
                  </Field>
                  <Field
                    label="Prix unitaire HT (€)"
                    error={(form.formState.errors as any).lines?.[i]?.prix_unitaire_ht?.message}
                  >
                    <Input
                      type="number"
                      step="0.01"
