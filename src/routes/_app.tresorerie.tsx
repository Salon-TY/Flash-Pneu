/* eslint-disable @typescript-eslint/no-explicit-any -- agrégations financières historiques, calculs inchangés */
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [current, prev, overdueInvoices, allInvoices6m] = await Promise.all([
        db
          .from("invoices")
          .select("*, client:clients(id, raison_sociale, email)")
          .gte("date_facture", start)
          .lte("date_facture", end),
        db
          .from("invoices")
          .select("total_ttc, statut")
          .gte("date_facture", prevStart)
          .lte("date_facture", prevEnd),
        db
          .from("invoices")
          .select("*, client:clients(id, raison_sociale, email)")
        ...
        db
          .from("invoices")
          .select("total_ttc, statut, client_id, date_facture, client:clients(id, raison_sociale)")
          .gte(
            "date_facture",
      ...
      const caEncaisse = inv
        .filter((i: any) => i.statut === "payee")
        .reduce((s: number, i: any) => s + Number(i.total_ttc ?? 0), 0);
      const caAttente = inv
        .filter((i: any) => i.statut === "envoyee")
        .reduce((s: number, i: any) => s + Number(i.total_ttc ?? 0), 0);
      const caRetard = inv
        .filter((i: any) => i.statut === "retard")
        .reduce((s: number, i: any) => s + Number(i.total_ttc ?? 0), 0);
      const caPrevu = inv
        .filter((i: any) => i.statut === "brouillon")
        .reduce((s: number, i: any) => s + Number(i.total_ttc ?? 0), 0);
      const caTotal = inv
        .filter((i: any) => PAID.includes(i.statut))
        .reduce((s: number, i: any) => s + Number(i.total_ttc ?? 0), 0);
      const prevTotal = (prev.data ?? [])
        .filter((i: any) => PAID.includes(i.statut))
        .reduce((s: number, i: any) => s + Number(i.total_ttc ?? 0), 0);
      ...
      const relances = (overdueInvoices.data ?? [])
        ...
        .map((i: any) => ({
          ...i,
          daysLate: Math.floor(
            (now.getTime() - new Date(i.echeance + "T00:00:00").getTime()) / (1000 * 60 * 60 * 24),
          ),
        }))
      ...
      const clientMap = new Map<
        string,
        { id: string; nom: string; payee: number; attente: number; retard: number }
      >();
      for (const i of inv) {
      ...
      const clientStats = [...clientMap.values()].sort(
        (a, b) => b.payee + b.attente + b.retard - (a.payee + a.attente + a.retard),
      );
      const top5 = clientStats
        .slice(0, 5)
        .map((c) => ({ ...c, ca: c.payee + c.attente + c.retard }));
...
function TresoreriePage() {
  ...
  const { data, isLoading, isError } = useTresorerieData(period);
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Trésorerie"
        subtitle="Suivi financier de votre activité."
        actions={
          can("export") ? (
      ...
      {/* KPI principal */}
      <div className="rounded-2xl bg-primary p-4 text-primary-foreground sm:p-6">
        <div className="mb-1 text-xs uppercase tracking-widest text-primary-foreground/60">
          CA encaissé
        </div>
        ...
        <div className="mt-1 flex items-center gap-1.5 text-sm text-primary-foreground/80">
          {evolution !== null && evolution !== undefined ? (
            <>
              ...
              <span
                className={evolution > 0 ? "text-success" : evolution < 0 ? "text-destructive" : ""}
              >
                {evolution > 0 ? "+" : ""}
                {Math.round(evolution)}% vs période précédente
              </span>
            </>
          ) : (
            <span className="text-primary-foreground/50">Pas de données précédentes</span>
          )}
        </div>
        {prevTotal > 0 && (
          <div className="mt-3">
            <div className="h-1.5 w-full overflow-hidden rounded-full bg-primary-foreground/20">
              <div
                ...
                style={{
                  width: `${Math.min(100, caTotal > 0 && prevTotal > 0 ? (caTotal / Math.max(caTotal, prevTotal)) * 100 : 0)}%`,
                }}
              />
            </div>
            <div className="mt-1 flex justify-between text-[10px] text-primary-foreground/60">
        ...
        {[
          {
            key: "payee",
            label: "Payé",
            value: data?.caEncaisse ?? 0,
            color: "bg-success/10",
            textColor: "text-success",
            invoices: data?.invoicesByStatus.payee ?? [],
          },
          {
            key: "envoyee",
            label: "En attente",
            value: data?.caAttente ?? 0,
            color: "bg-warning/10",
            textColor: "text-warning-foreground",
            invoices: data?.invoicesByStatus.envoyee ?? [],
          },
          {
            key: "retard",
            label: "En retard",
            value: data?.caRetard ?? 0,
            color: "bg-destructive/10",
            textColor: "text-destructive",
            invoices: data?.invoicesByStatus.retard ?? [],
          },
          {
            key: "brouillon",
            label: "Prévu",
            value: data?.caPrevu ?? 0,
            color: "bg-muted",
            textColor: "text-muted-foreground",
            invoices: data?.invoicesByStatus.brouillon ?? [],
          },
        ].map(({ key, label, value, color, textColor, invoices }) => (
          <div key={key} className="col-span-1">
            <div
              className={`rounded-xl p-3 ${color} cursor-pointer`}
              ...
              <Card className="mt-1">
                <CardContent className="p-0">
                  {invoices.map((inv: any, i: number) => {
                    const daysLate =
                      key === "retard" && inv.echeance
                        ? Math.floor(
                            (new Date().getTime() -
                              new Date(inv.echeance + "T00:00:00").getTime()) /
                              (1000 * 60 * 60 * 24),
                          )
                        : null;
                    const soonDue =
                      key === "envoyee" && inv.echeance
                        ? Math.floor(
                            (new Date(inv.echeance + "T00:00:00").getTime() -
                              new Date().getTime()) /
                              (1000 * 60 * 60 * 24),
                    ...
                    return (
                      <div
                        key={inv.id}
                        className={`flex items-center gap-2 px-3 py-2 text-xs ${i < invoices.length - 1 ? "border-b" : ""}`}
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-muted-foreground flex items-center gap-1.5">
                            ...
                            {soonDue !== null && soonDue <= 7 && soonDue >= 0 && (
                              <span className="rounded-full bg-warning/10 px-1.5 py-0.5 text-[9px] font-semibold text-warning-foreground">
                                Bientôt dû
                              </span>
                            )}
                          </div>
                        ...
                        {key === "retard" && (inv.client as any)?.email && (
                          <a
                            href={`mailto:${(inv.client as any).email}?subject=Relance%20facture&body=Bonjour%2C%20merci%20de%20r%C3%A9gler%20votre%20facture%20de%20${formatEUR(inv.total_ttc)}.`}
              ...
              >
                ...
                <Bar dataKey="ca" radius={[4, 4, 0, 0]}>
                  {(data?.months6 ?? []).map((entry, i) => (
                    <Cell
                      key={i}
                      fill={entry.current ? "var(--primary)" : "var(--muted-foreground)"}
                    />
                  ))}
                </Bar>
              </BarChart>
        ...
        <section>
          ...
          <div className="space-y-2">
            {data!.relances.map((inv: any) => (
              <Card key={inv.id} className="border-destructive/30">
                <CardContent className="p-3 flex items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground flex flex-wrap items-center gap-1.5">
                      ...
                      {(() => {
                        ...
                        return sent > 0 ? (
                          <span
                            className={`rounded-full px-1.5 py-0.5 text-[9px] font-semibold ${NIVEAU_RELANCE_COLORS[sent] ?? ""}`}
                          >
                            N{sent} envoyé
                          </span>
                        ) : (
                          <span
                            className={`rounded-full px-1.5 py-0.5 text-[9px] font-semibold ${NIVEAU_RELANCE_COLORS[niveau]}`}
                          >
                            N{niveau} à envoyer
                          </span>
                        );
                  ...
                  <div className="flex items-center gap-2">
                    <a
                      href={`mailto:${(inv.client as any)?.email ?? ""}?subject=Relance%20facture&body=Bonjour%2C%20nous%20vous%20relançons%20pour%20la%20facture%20de%20${formatEUR(inv.total_ttc)}%20%C3%A9chue%20depuis%20${inv.daysLate}%20jours.`}
                      className="flex min-h-10 items-center gap-1.5 rounded-lg bg-destructive/10 px-3 py-2 text-xs font-semibold text-destructive transition-colors hover:bg-destructive/15"
                    >
                      <Mail className="h-3.5 w-3.5" />
                      Relancer
                    </a>
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground flex gap-2 mt-0.5">
                        {c.payee > 0 && (
                          <span className="text-success">Payé {formatEUR(c.payee)}</span>
                        )}
                        {c.attente > 0 && (
                          <span className="text-warning-foreground">
                            Att. {formatEUR(c.attente)}
                          </span>
                        )}
                        {c.retard > 0 && (
                          <span className="text-destructive">Ret. {formatEUR(c.retard)}</span>
                        )}
                      </div>
                    </div>
